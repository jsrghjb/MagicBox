...
export class DeviceScanner {
  ...
  async startScanningMultiple(types, callback) {
    ...
    try {
      ...
      console.warn('DeviceScanner startScanningMultiple warning:', e?.message || e)
    }
  }
...
function createDeviceKey(device) {
  const { host, port } = parseDeviceId(device?.address || '')
  const targetPort = Number(device?.port) || port || 5555
  return `${host || device?.address}:${targetPort}`
}
...
export function getHistoryWirelessDevices() {
  try {
    const devices = electronStore.get('device') || {}
    const list = []

    for (const [key, item] of Object.entries(devices)) {
      const isWifi = item.wifi || ([':', '_adb-tls-connect']).some(str => key.includes(str))
      if (isWifi) {
        const { host, port } = parseDeviceId(key)
        if (host && net.isIP(host) === 4) {
          list.push(new DeviceData(item.remark || item.name || host, host, port || 5555, 'history'))
        }
      }
    }
    ...
    console.warn('getHistoryWirelessDevices warning:', error?.message || error)
    return []
  }
}
...
export async function scanMdnsDevices(options = {}) {
  const {
    types = [MDNS_CONFIG.CONNECT_TYPE, MDNS_CONFIG.LEGACY_ADB_TYPE, MDNS_CONFIG.PAIRING_TYPE],
