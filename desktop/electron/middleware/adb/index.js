...
async function watch(callback) {
  ...
  tracker.on('change', async (ret, oldDevice) => {
    callback('change', ret, oldDevice)
  })
...
async function pair(host, port, code) {
  const raw = port ? `${host}:${port}` : host
  const { host: targetHost, port: targetPort } = parseDeviceId(raw)
...
async function disconnect(host, port) {
  ...
  const errorKeys = ['error', 'failed', 'cannot', 'unable', 'no such']

  if (errorKeys.some(item => String(stdout || '').toLowerCase().includes(item))) {
    const altResult = await shell(`disconnect ${targetHost}`)
    const altOut = String(altResult.stdout || '')
    if (errorKeys.some(item => altOut.toLowerCase().includes(item))) {
      throw stdout || altOut
    }
  }
