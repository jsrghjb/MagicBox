...
export async function removeDevices(...devices) {
  ...
  for (const device of devices) {
    ...
    if (id) {
      const isWifi = typeof device === 'object' ? device.wifi : ([':', '_adb-tls-connect']).some(item => id.includes(item))
      if (isWifi) {
        try {
          await window.$preload.adb.disconnect(id)
        }
        catch (error) {
          console.warn(`Failed to disconnect device ${id}:`, error)
        }
      }
